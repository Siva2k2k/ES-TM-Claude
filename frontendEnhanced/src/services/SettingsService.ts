...
export class SettingsService {
   ...
   */
  static async updateNotifications(notifications: Partial<UserSettings['notifications']>): Promise<{ success: boolean; error?: string }> {
    try {
      const response = await backendApi.put<{ success: boolean; error?: string }>('/settings/notifications', notifications);
      return { success: response.success, error: response.error };
    } catch (error) {
   ...
   */
  static async resetUserSettings(): Promise<{ settings?: UserSettings; error?: string }> {
    try {
      const response = await backendApi.post<{ success: boolean; settings?: UserSettings; error?: string }>('/settings/profile/reset');
      
      if (response.success && response.settings) {
        return { settings: response.settings };
      } else {
        return { error: response.error || 'Failed to reset settings' };
      }
    } catch (error) {
