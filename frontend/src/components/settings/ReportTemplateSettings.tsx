...
export const ReportTemplateSettings: React.FC<ReportTemplateSettingsProps> = ({
  onSettingsChange,
  onSettingsSaved
}) => {
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500">Create and manage custom report templates.</p>
        </div>
      ...
      {showCreateForm && (
        <div className="bg-gray-50 border border-gray-200 rounded-lg p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <label htmlFor="template-format" className="block text-sm font-medium text-gray-700 mb-1">
                Format
              </label>
              <select
                id="template-format"
                value={formData.format}
                onChange={(e) => setFormData(prev => ({ ...prev, format: e.target.value as 'pdf' | 'excel' | 'csv' }))}
                className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500"
              >
